...
def color2grey(image, weights=[0.21, 0.72, 0.07]):
    ...
    G = np.empty(shape=(imshape[0], imshape[1]), dtype='uint8')
...
if __name__ == '__main__':
    ...
    num_runs = 100
    avg_runtime = timeit.timeit("color2grey(image)", globals=locals(), number = num_runs) / num_runs

    with open("reports/%s_report_color2gray.txt" % __file__.split("_")[0], "w") as file:
        file.write("Timing: %s\n" % __file__)
        file.write("Average runtime running %s after %i runs: %.3e s\n" % (__file__, num_runs, avg_runtime))
