...
def python_benchmark(num_runs=3):
    ...
    cv2.imwrite("img/rain_sepia_python.jpeg", cv2.cvtColor(python_color2sepia.color2sepia(image), cv2.COLOR_RGB2BGR))
    return
...
def numpy_benchmark(num_runs=3):
    ...
    with open("reports/numpy_report_color2sepia.txt", "w") as file:
        ...
        # Fetch runtime of pure python implementation by parsing the report
        python_runtime = get_runtime_from_report("reports/python_report_color2sepia.txt")
    ...
    # Save a copy of the image as a quick "unit test"
    # Also remembering to convert from RGB representation to BGR which is standard in cv2
    cv2.imwrite("img/rain_sepia_numpy.jpeg", cv2.cvtColor(numpy_color2sepia.color2sepia(image), cv2.COLOR_RGB2BGR))
...
def numba_benchmark(num_runs=3):
    ...
    with open("reports/numba_report_color2sepia.txt", "w") as file:
        ...
        numpy_runtime = get_runtime_from_report("reports/numpy_report_color2sepia.txt")
        file.write("Average runtime running of numba_color2sepia is ")
...
"""
    ...
    # Save a copy of the image as a quick "unit test"
    # Also remembering to convert from RGB representation to BGR which is standard in cv2
    cv2.imwrite("img/rain_sepia_numba.jpeg", cv2.cvtColor(numba_color2sepia.color2sepia(image), cv2.COLOR_RGB2BGR))
    return
...
def cython_benchmark(num_runs=3):
    ...
    with open("reports/cython_report_color2sepia.txt", "w") as file:
        file.write("Size of image: %ix%i\n" % (image.shape[1], image.shape[0]))
        file.write("Timing: %s\n" % __file__)
        file.write(
            "Average runtime running %s after %i runs: %.6f s\n"
        ...
        numba_runtime = get_runtime_from_report("reports/numba_report_color2sepia.txt")
        file.write("Average runtime running of cython_color2sepia is ")
...
Comment:
    ...
    # Save a copy of the image as a quick "unit test"
    # Also remembering to convert from RGB representation to BGR which is standard in cv2
    cv2.imwrite("img/rain_sepia_cython.jpeg", cv2.cvtColor(cython_color2sepia.color2sepia(image), cv2.COLOR_RGB2BGR))
